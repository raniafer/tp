...
# plots pour le projet
'''

90 for i in range(0, nt, 1000):
 91         plt.plot(x, u_values[i, :], label=f'{i}')
 92 plt.legend()
 93 plt.show()
 94 
 95 # plot the result
 96 fig, ax = plt.subplots(figsize=(10,8))
 97 
 98 xx, tt = np.meshgrid(x, t)
 99 levels = np.arange(-3, 3, 0.01)
100 cs=ax.contourf(xx, tt, u_values, cmap=cm.jet)
101 fig.colorbar(cs)
102 
103 ax.set_xlabel("x")
104 ax.set_ylabel("t")
